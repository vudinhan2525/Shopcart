...
function Invoice({ userData }) {
  ...
  return (
    <div className="px-10 py-10 dark:text-dark-text">
      <header className="text-2xl font-bold">Invoices</header>
      <p className="text-sm text-gray-600 font-normal">See all your invoices here</p>
      <div>
        {isEmpty && <NoResultFound />}
        {products.map((el, idx) => {
          return (
            <div className="px-6 py-4 border-[1px] dark:border-gray-700 border-gray-400 rounded-xl mt-4" key={idx}>
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="bg-gray-300 dark:bg-gray-700 w-[1px] h-[20px]"></div>
                  <p className="text-orange-600 font-semibold ">
                    {billData[idx].status === 'pending' ? 'PENDING' : 'COMPLETED'}
                  </p>
                </div>
              </div>
              <div className="w-full h-[1px] bg-gray-300 dark:bg-gray-700 my-2"></div>
              <Link to={`/product/${el._id}`} className="flex gap-4">
                <div
                  style={{ backgroundImage: `url(${el?.images[0]})` }}
                  className="h-[80px] bg-white border-[1px] border-gray-500 w-[80px] bg-no-repeat bg-center  bg-contain rounded-lg"
                ></div>
                <div className="basis-[75%]">
                  <p className="text-lg font-semibold">{el.name}</p>
                  <div className="text-sm text-gray-700 dark:text-gray-500">{`x${billData[idx].products.quantity}`}</div>
                </div>
